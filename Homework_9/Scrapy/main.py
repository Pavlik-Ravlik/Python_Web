...
class AuthorsSpider(scrapy.Spider):
    name = 'authors'
    custom_settings = {"FEED_FORMAT": "json", "FEED_URI": "jsons/authors.json", 'FEED_EXPORT_INDENT': 4}
    allowed_domains = ['quotes.toscrape.com']
    start_urls = ['http://quotes.toscrape.com/']

    def parse(self, response):
        for author_div in response.css("div.quote"):
            author_info = {"fullname": author_div.css("small.author::text").get(),}
...
class QuotesSpider(scrapy.Spider):
    name = 'quotes'
    custom_settings = {"FEED_FORMAT": "json", "FEED_URI": "jsons/quotes.json", 'FEED_EXPORT_INDENT': 4}
    allowed_domains = ['quotes.toscrape.com']
    start_urls = ['http://quotes.toscrape.com/']
