...
#-----------------------------------------------------------------------------------------------------------------------------------
    async def test_create_contact(self):
        body = ContactRequest(first_name='Pavlo', last_name='Pupkin', email='example@gmail.com', phone_number='380777777777', birthday=date(2023, 9, 4))
        self.session.query.return_value.filter.return_value.first.return_value = None  # Никакой контакт с таким номером не существует
...
#-----------------------------------------------------------------------------------------------------------------------------------
    async def test_get_contacts(self):
        ...
        result = await get_contacts(db=self.session, current_user=self.current_user)
...
#-----------------------------------------------------------------------------------------------------------------------------------
    async def test_update_contact(self):
        contact_id = 1
        contact = Contact(id=contact_id, user_id=self.current_user.id)
        updated_contact_data = ContactRequest(first_name='Updated', last_name='Contact', email='updated@example.com', phone_number='12345', birthday=date(2023, 9, 4))
        self.session.query.return_value.filter.return_value.first.return_value = contact
        result = await update_contact(contact_id=contact_id, updated_contact=updated_contact_data, db=self.session, current_user=self.current_user)
        ...
        self.session.query.return_value.filter.return_value.first.return_value = None
        with self.assertRaises(HTTPException) as context:
            await update_contact(contact_id=contact_id, updated_contact=updated_contact_data, db=self.session, current_user=self.current_user)
        self.assertEqual(context.exception.status_code, 404)
        self.assertEqual(context.exception.detail, "Contact not found")
